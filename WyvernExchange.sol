...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    function staticCall(
        ...
        bytes memory combined = new bytes(
            SafeMath.add(calldata.length, extradata.length)
        );
    ...
    function hashToSign(Order memory order) internal pure returns (bytes32) {
        ...
        bytes32 hash = keccak256(
            prefix,
            hashOrderPartOne(order),
            hashOrderPartTwo(order)
        );
        return hash;
    }
...
contract AuthenticatedProxy is TokenRecipient {
     ...
     */
    function AuthenticatedProxy(address addrUser, ProxyRegistry addrRegistry)
        public
    {
        user = addrUser;
